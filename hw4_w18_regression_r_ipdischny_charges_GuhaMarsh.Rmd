...
## Top 1 Model: Model 13, MAD 5745.559
```{r}
...
## Top 2 Model: Model 10, MAD: 5887.513
```{r}
...
## Top 3 Model: Model 9, MAD: 5932.954
```{r}
...
# Cleaning up the data to make them more representable with better column names
```{r}
names(cvResults) <- c("Error", "Adjusted Error")
cvResults$Model <- sprintf("totalChargesG%s",9:13)
cvResults
```
Since, while running the k-crossfold validation we identified that our glm and lm were not equal and hence it makes sense that this time we are getting our Top 3 Models different. The lower the error, the better the Model Fit. Hence, according to k-crossfold validation, our Top 3 Models are: 
1. Model 10
2. Model 9
3. Model 13
...
# Top 1 Predictive Model, Test MAD: 5752.152
```{r}
...
# Top 2 Predictive Model, Test MAD: 5856.165
```{r}
...
# Top 3 Predictive Model, Test MAD: 5888.697
```{r}
...
Model 13 has the lowest MAD value for the test Data.

The Top 3 Models with the MAD for train and test data are as follows, with MAD values in ascending order:
1. Model 13, Train MAD: 5745.559, Test MAD: 5752.152
2. Model 10, Train MAD: 5887.513, Test MAD: 5856.165
3. Model 9, Train MAD: 5932.954, Test MAD: 5888.697
...
ggplot(data = df4) + geom_point(aes(x = trainMads, y = testMads))
